...
class GeneratoreCodice:
    ...
    def genera(self, frase: str) -> str:
        ...
        1. Prova con AI locale (gestisce anche multi-comando)
        2. Controlla se è multi-comando
        3. Se fallisce, usa sistema a regole
        4. Se anche quello fallisce, ritorna messaggio di errore
        
        Args:
            frase: frase in italiano (può contenere più comandi)
        ...
        # Strategia 2: Multi-comando con regole
        if self.use_fallback:
            # Prova prima multi-comando
            from .multi_comando import MultiComandoParser
            parser_multi = MultiComandoParser()
            
            if parser_multi.identifica_multi_comando(frase):
                codice = self._genera_multi_comando(frase, parser_multi)
                if codice and not codice.startswith("# Comando"):
                    self.logger.info("✅ Codice multi-comando generato con regole")
                    if self.use_cache and self.cache:
                        self.cache.set(frase, codice)
                    return codice
            
            # Fallback: singolo comando
