...
class PythonitaGUI3D:
    ...
    def __init__(self, root):
        ...
        # Generatore con template robot (AI disabilitata per velocita')
        try:
            self.generatore = GeneratoreCodice(template='robot', use_ai=False, use_cache=False)
        except Exception as e:
            print(f"[WARNING] Errore inizializzazione generatore: {e}")
            print("[INFO] Uso generatore base senza template")
            self.generatore = GeneratoreCodice(use_ai=False, use_cache=False)
