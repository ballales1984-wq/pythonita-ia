...
class LicenseManager:
    ...
    def validate_key(self, key: str) -> Tuple[bool, Tier, str]:
        ...
        # Validazione formato chiave (offline-first)
        # Formato: TIER-HASH-CHECKSUM
        # Esempio: PRO-A1B2C3D4-E5F6
        if self._validate_key_format(key_clean):
            tier, valid = self._extract_tier_from_key(key_clean)
            if valid:
                logger.info(f"Licenza {tier} validata (offline)")
                return True, tier, f"Licenza {tier.value} valida"
        
        # Fallback: chiavi di test per sviluppo
        test_keys = {
        ...
        if key_clean in test_keys:
            ...
            logger.warning("Usando chiave di test (dev only)")
            return True, tier, f"Licenza {tier.value} di test"
        
        # TODO Future: Online validation con API server
        # if self._validate_with_server(key_clean):
        #     return True, tier, "Licenza validata online"
    ...
    def _validate_key_format(self, key: str) -> bool:
        """
        Valida formato chiave licenza.
        
        Formato atteso: 16 caratteri hex (senza trattini) o
        formato strutturato TIER-HASH-CHECK
        """
        ...
        # Deve contenere solo alfanumerici
        if not key.replace("-", "").isalnum():
            return False
        
        return True
    
    def _extract_tier_from_key(self, key: str) -> Tuple[Tier, bool]:
        """
        Estrae tier da chiave e valida checksum.
        
        Algoritmo:
        1. Primo carattere codifica tier (P=Personal, R=Pro, E=Enterprise)
        2. Ultimi 4 caratteri = checksum
        3. Checksum = SHA256(resto + SECRET)[:4]
        """
        ...
        # Estrai tier da primo carattere
        tier_map = {
            'P': Tier.PERSONAL,
            'R': Tier.PRO,        # PRO usa 'R' per evitare confusione con Personal
            'E': Tier.ENTERPRISE,
            'T': Tier.TRIAL,
        }
        
        first_char = key[0]
        if first_char not in tier_map:
            return Tier.FREE, False
        
        tier = tier_map[first_char]
        
        # Estrai payload e checksum
        payload = key[:-4]
        checksum = key[-4:]
    ...
    def _calculate_checksum(self, payload: str) -> str:
        """Calcola checksum per validazione chiave."""
        data = f"{payload}{self.SECRET_KEY}".encode()
        hash_obj = hashlib.sha256(data)
        return hash_obj.hexdigest()[:4].upper()
    
    def generate_license_key(self, tier: Tier, customer_id: str = "") -> str:
        """
        Genera chiave licenza valida per il tier specificato.
        
        NOTA: Questo metodo dovrebbe essere usato SOLO lato server
        per generare chiavi reali. Incluso qui per testing.
        ...
        tier_char = tier_chars.get(tier, 'F')
        
        # Genera payload random
        random_part = uuid.uuid4().hex[:11].upper()
        payload = f"{tier_char}{random_part}"
        
        # Calcola checksum
        checksum = self._calculate_checksum(payload)
        
        # Combina
        full_key = f"{payload}{checksum}"
        
        # Formatta come XXXX-XXXX-XXXX-XXXX
        formatted = "-".join([full_key[i:i+4] for i in range(0, 16, 4)])
        
        logger.info(f"Generata chiave {tier.value}: {formatted}")
